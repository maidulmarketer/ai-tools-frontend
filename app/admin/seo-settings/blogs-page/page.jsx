...
export default function BlogsPageSeo() {
  const { isLoading, data, refetch, isError } = useQuery({
    queryKey: ["blogs-page-seo"],
  ...
  const {
    ...
    values: data?.blogs_page,
  });

  function onSubmit(data) {
    const promise = updateStorage({ blogs_page: data }).then(refetch);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-start gap-4">
        <BackButton />
        <PageTitle title="SEO Settings for Blogs Page" />
      </div>
